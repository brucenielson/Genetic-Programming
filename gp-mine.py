...
class constnode:
    ...
    def display(self, indent=0):
        print '%s%d' % (' '*indent, self.constant)
...
def make_random_tree(pc, max_depth=4, func_prob=0.5, param_prob=0.6):
    if random() < func_prob and max_depth >0:
        f = choice(flist)
        children = [make_random_tree(pc, max_depth-1, func_prob, param_prob)
